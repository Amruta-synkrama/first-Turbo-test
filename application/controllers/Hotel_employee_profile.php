...
class Hotel_employee_profile extends CI_Controller
{
	public function __construct() {
		...
		if(!$this->session->userdata('user_data')) {
			redirect('dashboard');	
		} elseif($this->session->userdata('user_data')->entity != 'Hotel_EMP' && $this->session->userdata('user_data')->entity != 'Hotel' && $this->session->userdata('user_data')->entity != 'Admin') {
			redirect('dashboard');
		}
	...
	public function index() {
		...
		if($this->session->userdata('user_data')->entity == 'Admin' || $this->session->userdata('user_data')->entity == 'Hotel') {
		...
		$data['hotels_emp_data'] = $this->user_model->get_hotel_emp_data($emp_id);
		$data['emp_id'] = $emp_id;
		$this->load->view('templates/header', $data);
		$this->load->view('hotel_employee_profile', $data);
	...
	public function update_user_details() {
		...
		$user_data = $this->user_model->get_user_data($emp_id);
		...
		if($this->form_validation->run()) {
			...
			if($this->user_model->update_user_data($data, $emp_id)) {
				...
				if($this->session->userdata('user_data')->entity == 'Admin' || $this->session->userdata('user_data')->entity == 'Hotel') {
					redirect('hotel_employee_profile?emp_id='.$emp_id);
				}else {
					redirect('hotel_employee_profile');
				}
			} else {
				if($this->session->userdata('user_data')->entity == 'Admin' || $this->session->userdata('user_data')->entity == 'Hotel') {
					redirect('hotel_employee_profile?emp_id='.$emp_id);
	...
	*/
	...
	public function update_employee_details() {
		...
		if($this->form_validation->run()) {
			...
			if($this->user_model->update_hotel_emp_data($data, $emp_id)) {
				// $this->session->set_flashdata('user_message', 'Data updated');
				$this->session->set_flashdata('success_message', 'Data updated');
				if($this->session->userdata('user_data')->entity == 'Admin' || $this->session->userdata('user_data')->entity == 'Hotel') {
					redirect('hotel_employee_profile?emp_id='.$emp_id);
...
/* End of file '/Hotel_employee_profile.php' */
/* Location: ./application/controllers//Hotel_employee_profile.php */
